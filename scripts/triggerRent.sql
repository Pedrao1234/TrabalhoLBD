create or replace function on_create_rent() 
returns TRIGGER as $$ 
begin 
    insert into log_register (name_table, id_entity, created_date) values ('rent', new.id, CURRENT_DATE);
	return new;
end 
$$ language plpgsql;

create trigger pre_create_rent
before insert on rent
for each row 
execute function on_create_rent();

create or replace function on_update_rent() 
returns trigger as $$
begin 
	update log_register
	set last_modified_date = CURRENT_DATE
	where id_entity = new.id;
	
	return new;
end
$$ language plpgsql;

create trigger pre_update_rent
before update on rent
for each row
execute function on_update_rent();

create or replace function on_delete_rent() 
returns trigger as $$
begin 
	update log_register
	set deleted_date = CURRENT_DATE
	where id_entity = old.id;
	return old;
end
$$ language plpgsql;

create trigger pre_delete_rent
before delete on rent
for each row
execute function on_delete_rent();